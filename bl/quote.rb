...
# 	else
# 		full_page_card(:"answer_quote", locals: {quote_id: params['_id'], })
# 	end
...
get '/answer_seller' do
	...
	#text =  $sms.get(type:"send_quote", )[:text]
...
post '/answer_quote' do
	...
	quote = $quotes.get(_id:quote_id) #escue flash.message = "Your link is broken. Please follow again link from sms" #(????)
  	...
  	if seller_phone.in?(quote[:answered_sellers])
		...
		if seller_phone.in?(sellers_sent_to)
				...
				#$quotes.update_id(params[:quote_id], {answered_sellers: seller_phone})
				$quotes.find_one_and_update({_id: params[:quote_id]}, {'$push' => {answered_sellers: seller_phone}})    
				# update quote if this user already answered it with answered_sellers: sellers_sent_to. then check in the beginning if
				# user hasn't yet answered
...
post '/create_quote' do 
 		...
 		treatments = params['treatments'][0].present? ? params['treatments'] : ["Manicure"] # condition ?if condition true : if cond falsee
 		...
 		sellers_sent_to = get_users_around(latitude, longitude, treatments) 
		quote  = $quotes.add({
		buyer_name:cu[:name], 
...
post '/add_sms' do 
		...
		sellers_sent_to = get_users_around(params['latitude'], params['longitude'], treatments) 
